...
router.post("/:courseId/join-forum", async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Access granted to course forum.", forumId: forum._id });
  } catch (err) {
...
router.get("/:courseId/messages", async (req, res) => {
  try {
    ...
    const forum = await Forum.findOne({ courseId }).populate("messages.sender", "name");
